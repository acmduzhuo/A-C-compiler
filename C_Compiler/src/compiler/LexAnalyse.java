...
public class LexAnalyse {
	...
	private static boolean isDigit(char ch) {//数字判断
		boolean flag = false;
		if ('0' <= ch && ch <= '9')
			flag = true;
		return flag;
	...
	private static boolean isInteger(String word) {//单词int？
		int i;
		boolean flag = false;
	...
	private static boolean isChar(String word) {//单词常量？
		boolean flag = false;
		int i = 0;
	...
	private static boolean isLetter(char ch) {//字符字母？
		boolean flag = false;
	...
	private static boolean isID(String word) {//单词合法？
		boolean flag = false;
		int i = 0;
		if (Word.isKey(word))
			return flag;
	...
	public boolean isFail() {//词法分析通过检测
		return lexErrorFlag;
	}
